...
from realtime_uploader import RealtimeExoBackup
...
# Real-time Cloud / Local Buffer Backup Config
ENABLE_HF_BACKUP = True
HF_REPO_ID = "Amilyl/mrsd_ankle_exo_data"
HF_TOKEN = os.getenv("HF_TOKEN", "")
BUFFER_SEC = 5.0                               # 每 5 秒批次寫入磁碟與雲端
...
class SensorHub:
    ...
    def __init__(
        ...
        if MOCK_HARDWARE:
            print("[Hub] ⚠️ MOCK_HARDWARE Mode Enabled (Skipping physical I2C connection)")
            return
    ...
    def read_fsr(self) -> dict:
        if MOCK_HARDWARE:
            return {"toe_fsr_raw": 512, "heel_fsr_raw": 1024}   
        self._ads.update()
        return {
...
def main() -> None:
    ...
    # Initialize Realtime Cloud Backup worker if enabled
    backup_worker = None
    if ENABLE_HF_BACKUP and RealtimeExoBackup is not None:
        backup_worker = RealtimeExoBackup(
            repo_id=HF_REPO_ID,
            hf_token=HF_TOKEN,
            buffer_sec=BUFFER_SEC,
            local_dir=str(OUTPUT_DIR / "realtime_batches")
        )
    ...
    with SensorHub() as hub, out_file.open("w", newline="") as f:
        ...
        try:
            while not stop_requested:
                ...
                # Feed frame to background backup worker
                if backup_worker is not None:
                    frame_dict = dict(zip(header, row))
                    backup_worker.log_sensor_frame(frame_dict)
            ...
            # Flush and stop cloud backup worker
            if backup_worker is not None:
                backup_worker.stop()
